...
import { LayoutDashboard, Users, Gamepad2, Tag, LogOut, FileText, Activity, Bug, Settings } from 'lucide-react';
import { logoutAdmin } from '@/lib/api/auth';

interface SidebarProps {
  currentPage?: 'dashboard' | 'create-game' | 'users' | 'games' | 'categories' | 'pages' | 'ads' | 'traffic' | 'bug-reports' | 'settings';
  onNavigate?: (page: 'dashboard' | 'create-game' | 'users' | 'games' | 'categories' | 'pages' | 'ads' | 'traffic' | 'bug-reports' | 'settings') => void;
}
...
export default function Sidebar({ currentPage, onNavigate }: SidebarProps) {
  ...
  const handleNavigate = (href: string, page: 'dashboard' | 'create-game' | 'users' | 'games' | 'categories' | 'pages' | 'ads' | 'traffic' | 'bug-reports' | 'settings') => {
    router.push(href);
    onNavigate?.(page);
  };
    ...
    <aside className="w-full h-full bg-slate-900 border-r border-slate-800 flex flex-col">
      ...
      <nav className="flex-1 p-3 sm:p-4 space-y-2 overflow-y-auto">
        {menuItems.map((item) => {
          ...
          const isActive = pathname === item.href || (item.id === 'settings' && pathname === '/logos');
          return (
            <button
              key={item.id}
              onClick={() => handleNavigate(item.href, item.id as 'dashboard' | 'create-game' | 'users' | 'games' | 'categories' | 'pages' | 'ads' | 'traffic' | 'bug-reports' | 'settings')}
